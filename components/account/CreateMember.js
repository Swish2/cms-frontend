...
const {Item} = Form
const CreateMember = ({visible,setClose=()=>{}})=>{
    ...
    const [createMember,{loading}] = useMutation(CREATE_MEMBER,{
        update(cache,{data}){
            const newEntry = {...data?.createMember?.data}
    ...
    return(
        <Modal title="Add Member" visible={visible} onCancel={()=>setClose()} width={600} footer={null}>
            <Form layout="vertical" form={form} requiredMark={false} onFinish={(e)=>createMember({variables:e})}>
            <Row gutter={12}>
                <Col md={12} sm={24}>
               ...
               <Row gutter={12}>
                ...
                {
                    myProfile.role != "ADMIN" ?null:
                    <Item label="Cell" name="cellId">
                        <Select loading={loadingCell} options={cells}
                        placeholder="Select an option"/>
                    </Item>
                }
                
                <Item>
                    <Button style={{marginTop:20}} type="primary" block loading={loading} htmlType="submit">Add Member</Button>
                </Item>
            </Form>
        </Modal>
    )
